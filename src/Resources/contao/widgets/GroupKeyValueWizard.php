 ...
 */
class GroupKeyValueWizard extends Widget
{
	/**
	 * Submit user input
	 * @var boolean
	...
	public function validate()
	{
		...
		if (\is_array($options))
		{
			foreach ($options as $key=>$option)
			{
				...
				$options[$key]['group'] = trim($option['group']);
  ...
  <thead>
    <tr>
      <th>' . $GLOBALS['TL_LANG']['MSC']['ow_group'] . '</th>
  ...
  <tbody class="sortable">';
		...
		for ($i=0, $c=\count($this->varValue); $i<$c; $i++)
		{
			$return .= '
    <tr>
      <td><input type="text" name="' . $this->strId . '[' . $i . '][group]" id="' . $this->strId . '_key_' . $i . '" class="tl_text" value="' . StringUtil::specialchars($this->varValue[$i]['group'] ?? '') . '"' . $this->getAttributes() . '></td>
